  ...
  return cyberTerms
    .filter(item => {
      ...
      return (
        ...
        term.includes(searchValue.slice(0, 3))
      );
    })
    .slice(0, 6);
}
...
function renderTerms() {
  const searchValue = dictionarySearch.value.toLowerCase().trim();
  ...
  if (filteredTerms.length === 0) {
    termsGrid.innerHTML = `
      <div class="empty-state">
        <h3>No term found</h3>
        <p>Try another spelling or search a related word like attack, password, network, or malware.</p>
      </div>
    `;
    return;
  }
